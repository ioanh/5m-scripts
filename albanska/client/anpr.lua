		...
		if IsPedInAnyVehicle(myPed, false) then
			if GetPedInVehicleSeat(vehicle, -1) == myPed then
				if GetVehicleClass(vehicle) ~= 18 then
					...
					QBCore.Functions.TriggerCallback('albanska:IsPlateFlagged', function(result)
						if result then
							...
							TriggerServerEvent("albanska:server:FlaggedPlateTriggered", hasBeenBusted, plate, street1, street2, blipsettings)
						end
                    end, plate)
				end
			end
