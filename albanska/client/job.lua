    ...
    if coords then
        QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
            QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                ...
                TriggerServerEvent('albanska:server:TakeOutImpound',vehicle.plate)
                closeMenuFull()
...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        QBCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
            SetCarItemsInfo()
            SetVehicleNumberPlateText(veh, Lang:t('info.albanska_plate')..tostring(math.random(1000, 9999)))
            SetEntityHeading(veh, coords.w)
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'albanska' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "albanska:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("albanska:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "albanska:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
--Events
RegisterNetEvent('albanska:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('albanska:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(data)
    TriggerServerEvent('albanska:server:showFingerprintId', FingerPrintSessionId)
end)

RegisterNetEvent('albanska:client:SendEmergencyMessage', function(coords, message)
    TriggerServerEvent("albanska:server:SendEmergencyMessage", coords, message)
    TriggerEvent("albanska:client:CallAnim")
end)

RegisterNetEvent('albanska:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('albanska:client:CallAnim', function()
...
RegisterNetEvent('albanska:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            TriggerServerEvent("albanska:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
            QBCore.Functions.DeleteVehicle(vehicle)
        end
    end
end)

RegisterNetEvent('albanska:client:CheckStatus', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "albanska" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.TriggerCallback('albanska:GetPlayerStatus', function(result)
                    if result then
...
RegisterNetEvent("albanska:client:VehicleMenuHeader", function (data)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent("albanska:client:ImpoundMenuHeader", function (data)
...
RegisterNetEvent('albanska:client:TakeOutImpound', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['impound'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('albanska:client:TakeOutVehicle', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['vehicle'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('albanska:client:EvidenceStashDrawer', function(data)
...
RegisterNetEvent('qb-albanskajob:ToggleDuty', function()
    onDuty = not onDuty
    TriggerServerEvent("albanska:server:UpdateCurrentCops")
    TriggerServerEvent("albanska:server:UpdateBlips")
...
CreateThread(function()
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "albanska" then
            local pos = GetEntityCoords(PlayerPedId())
                            ...
                            exports['qb-menu']:showHeader({
                                {
                                    ...
                                    params = {
                                        event = 'albanska:client:EvidenceStashDrawer',
                                        args = {
                                            currentEvidence = k
                                        }
                                    }
                ...
                if #(pos - v) < 4.5 then
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "albanskastash_"..QBCore.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "albanskastash_"..QBCore.Functions.GetPlayerData().citizenid)
                            end
                        elseif #(pos - v) < 2.5 then
            ...
            for k, v in pairs(Config.Locations["trash"]) do
                if #(pos - v) < 2 then
                    ...
                    if #(pos - v) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("inventory:server:OpenInventory", "stash", "albanskatrash", {
                                maxweight = 4000000,
                                slots = 300,
                            })
                            TriggerEvent("inventory:client:SetCurrentStash", "albanskatrash")
                        end
                    elseif #(pos - v) < 1.5 then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                if player ~= -1 and distance < 2.5 then
                                    local playerId = GetPlayerServerId(player)
                                    TriggerServerEvent("albanska:server:showFingerprint", playerId)
                                else
                                    QBCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
                                end
                            end
                    ...
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                SetWeaponSeries()
                                TriggerServerEvent("inventory:server:OpenInventory", "shop", "albanska", authorizedItems)
                            end
                        elseif #(pos - v) < 2.5 then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    ...
                                    QBCore.Functions.SpawnVehicle(Config.albanskaHelicopter, function(veh)
                                    ...
                                    exports['qb-menu']:showHeader({
                                        {
                                            ...
                                            params = {
                                                event = 'albanska:client:ImpoundMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
                                    ...
                                    exports['qb-menu']:showHeader({
                                        {
                                            ...
                                            params = {
                                                event = 'albanska:client:VehicleMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
