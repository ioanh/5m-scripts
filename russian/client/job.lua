    ...
    if coords then
        QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
            QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                ...
                TriggerServerEvent('russian:server:TakeOutImpound',vehicle.plate)
                closeMenuFull()
...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        QBCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
            SetCarItemsInfo()
            SetVehicleNumberPlateText(veh, Lang:t('info.russian_plate')..tostring(math.random(1000, 9999)))
            SetEntityHeading(veh, coords.w)
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'russian' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "russian:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("russian:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "russian:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
--Events
RegisterNetEvent('russian:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('russian:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(data)
    TriggerServerEvent('russian:server:showFingerprintId', FingerPrintSessionId)
end)

RegisterNetEvent('russian:client:SendEmergencyMessage', function(coords, message)
    TriggerServerEvent("russian:server:SendEmergencyMessage", coords, message)
    TriggerEvent("russian:client:CallAnim")
end)

RegisterNetEvent('russian:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('russian:client:CallAnim', function()
...
RegisterNetEvent('russian:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            TriggerServerEvent("russian:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
            QBCore.Functions.DeleteVehicle(vehicle)
        end
    end
end)

RegisterNetEvent('russian:client:CheckStatus', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "russian" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.TriggerCallback('russian:GetPlayerStatus', function(result)
                    if result then
...
RegisterNetEvent("russian:client:VehicleMenuHeader", function (data)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent("russian:client:ImpoundMenuHeader", function (data)
...
RegisterNetEvent('russian:client:TakeOutImpound', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['impound'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('russian:client:TakeOutVehicle', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['vehicle'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('russian:client:EvidenceStashDrawer', function(data)
...
RegisterNetEvent('qb-russianjob:ToggleDuty', function()
    onDuty = not onDuty
    TriggerServerEvent("russian:server:UpdateCurrentCops")
    TriggerServerEvent("russian:server:UpdateBlips")
...
CreateThread(function()
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "russian" then
            local pos = GetEntityCoords(PlayerPedId())
                            ...
                            exports['qb-menu']:showHeader({
                                {
                                    ...
                                    params = {
                                        event = 'russian:client:EvidenceStashDrawer',
                                        args = {
                                            currentEvidence = k
                                        }
                                    }
                ...
                if #(pos - v) < 4.5 then
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "russianstash_"..QBCore.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "russianstash_"..QBCore.Functions.GetPlayerData().citizenid)
                            end
                        elseif #(pos - v) < 2.5 then
            ...
            for k, v in pairs(Config.Locations["trash"]) do
                if #(pos - v) < 2 then
                    ...
                    if #(pos - v) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("inventory:server:OpenInventory", "stash", "russiantrash", {
                                maxweight = 4000000,
                                slots = 300,
                            })
                            TriggerEvent("inventory:client:SetCurrentStash", "russiantrash")
                        end
                    elseif #(pos - v) < 1.5 then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                if player ~= -1 and distance < 2.5 then
                                    local playerId = GetPlayerServerId(player)
                                    TriggerServerEvent("russian:server:showFingerprint", playerId)
                                else
                                    QBCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
                                end
                            end
                    ...
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                SetWeaponSeries()
                                TriggerServerEvent("inventory:server:OpenInventory", "shop", "russian", authorizedItems)
                            end
                        elseif #(pos - v) < 2.5 then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    ...
                                    QBCore.Functions.SpawnVehicle(Config.russianHelicopter, function(veh)
                                    ...
                                    exports['qb-menu']:showHeader({
                                        {
                                            ...
                                            params = {
                                                event = 'russian:client:ImpoundMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
                                    ...
                                    exports['qb-menu']:showHeader({
                                        {
                                            ...
                                            params = {
                                                event = 'russian:client:VehicleMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
