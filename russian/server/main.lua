...
local function UpdateBlips()
    ...
    for k, v in pairs(players) do
        if (v.PlayerData.job.name == "russian" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
    ...
    TriggerClientEvent("russian:client:UpdateBlips", -1, dutyPlayers)
end
...
local function GetCurrentCops()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "russian" and v.PlayerData.job.onduty then
            amount = amount + 1
        end
    end
    return amount
...
QBCore.Commands.Add("spikestrip", Lang:t("commands.place_spike"), {}, false, function(source)
    ...
    if Player then
        if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
            TriggerClientEvent('russian:client:SpawnSpikeStrip', src)
        end
    end
end)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("pobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        if type == "cone" then
            TriggerClientEvent("russian:client:spawnCone", src)
        elseif type == "barrier" then
            TriggerClientEvent("russian:client:spawnBarrier", src)
        elseif type == "roadsign" then
            TriggerClientEvent("russian:client:spawnRoadSign", src)
        elseif type == "tent" then
            TriggerClientEvent("russian:client:spawnTent", src)
        elseif type == "light" then
            TriggerClientEvent("russian:client:spawnLight", src)
        elseif type == "delete" then
            TriggerClientEvent("russian:client:deleteObject", src)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cuff", Lang:t("commands.cuff_player"), {}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:CuffPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
...
QBCore.Commands.Add("escort", Lang:t("commands.escort"), {}, false, function(source, args)
    local src = source
    TriggerClientEvent("russian:client:EscortPlayer", src)
end)
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {{name = "id", help = Lang:t('info.player_id')}, {name = "time", help = Lang:t('info.jail_time')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        ...
        if time > 0 then
            TriggerClientEvent("russian:client:JailCommand", src, playerId, time)
        else
...
QBCore.Commands.Add("seizecash", Lang:t("commands.seizecash"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:SeizeCash", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
...
QBCore.Commands.Add("sc", Lang:t("commands.softcuff"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:CuffPlayerSoft", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cam", Lang:t("commands.camera"), {{name = "camid", help = Lang:t('info.camera_id')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:ActiveCamera", src, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:ImpoundVehicle", src, false, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
...
QBCore.Commands.Add("impound", Lang:t("commands.impound"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:ImpoundVehicle", src, true)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
    ...
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                OtherPlayer.Functions.AddMoney("bank", 500, "russian-tow-paid")
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                OtherPlayer.Functions.AddMoney("bank", 500, "russian-lawyer-paid")
                TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
...
QBCore.Commands.Add("anklet", Lang:t("commands.anklet"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:CheckDistance", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
        ...
        if args[1] then
            ...
            if Target then
                if Target.PlayerData.metadata["tracker"] then
                    TriggerClientEvent("russian:client:SendTrackerLocation", Target.PlayerData.source, src)
                else
...
QBCore.Commands.Add("takedrivinglicense", Lang:t("commands.drivinglicense"), {}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty then
        TriggerClientEvent("russian:client:SeizeDriverLicense", source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_russian_only"), 'error')
    end
end)
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if ((Player.PlayerData.job.name == "russian") and Player.PlayerData.job.onduty) and OtherPlayer then
...
RegisterNetEvent('russian:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("russian:client:TrackerMessage", requestId, msg, coords)
    TriggerClientEvent("qb-phone:client:addrussianAlert", requestId, alertData)
end)

QBCore.Commands.Add('911p', Lang:t("commands.russian_report"), {{name='message', help=Lang:t("commands.message_sent")}}, false, function(source, args)
	local src = source
    ...
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'russian' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addrussianAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('russian:client:russianAlert', v.PlayerData.source, coords, message)
        end
    end
end)
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    ...
    if Player.Functions.GetItemByName(item.name) then
        if item.info and item.info ~= "" then
            if Player.PlayerData.job.name ~= "russian" then
...
-- Callbacks
QBCore.Functions.CreateCallback('russian:server:isPlayerDead', function(source, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

QBCore.Functions.CreateCallback('russian:GetPlayerStatus', function(source, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
QBCore.Functions.CreateCallback('russian:IsSilencedWeapon', function(source, cb, weapon)
...
QBCore.Functions.CreateCallback('russian:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "russian" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('russian:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
QBCore.Functions.CreateCallback('russian:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
QBCore.Functions.CreateCallback('russian:GetCops', function(source, cb)
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "russian" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('russian:server:IsrussianForcePresent', function(source, cb)
    local retval = false
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "russian" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        CreateThread(function()
            MySQL.Async.execute("DELETE FROM stashitems WHERE stash='russiantrash'")
        end)
    end
end)

RegisterNetEvent('russian:server:russianAlert', function(text)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'russian' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addrussianAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('russian:client:russianAlert', v.PlayerData.source, coords, text)
        end
    end
end)

RegisterNetEvent('russian:server:TakeOutImpound', function(plate)
    local src = source
...
RegisterNetEvent('russian:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local CuffedPlayer = QBCore.Functions.GetPlayer(playerId)
    if CuffedPlayer then
        if Player.Functions.GetItemByName("handcuffs") or Player.PlayerData.job.name == "russian" then
            TriggerClientEvent("russian:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
        end
    end
end)

RegisterNetEvent('russian:server:EscortPlayer', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if (Player.PlayerData.job.name == "russian" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
            TriggerClientEvent("russian:client:GetEscorted", EscortPlayer.PlayerData.source, Player.PlayerData.source)
        else
...
RegisterNetEvent('russian:server:KidnapPlayer', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or
            ...
            TriggerClientEvent("russian:client:GetKidnappedTarget", EscortPlayer.PlayerData.source, Player.PlayerData.source)
            TriggerClientEvent("russian:client:GetKidnappedDragger", Player.PlayerData.source, EscortPlayer.PlayerData.source)
        else
            TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
        end
    end
...
RegisterNetEvent('russian:server:SetPlayerOutVehicle', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
            TriggerClientEvent("russian:client:SetOutVehicle", EscortPlayer.PlayerData.source)
        else
            TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
        end
    end
...
RegisterNetEvent('russian:server:PutPlayerInVehicle', function(playerId)
    local src = source
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
            TriggerClientEvent("russian:client:PutInVehicle", EscortPlayer.PlayerData.source)
        else
            TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
        end
    end
...
RegisterNetEvent('russian:server:BillPlayer', function(playerId, price)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if Player.PlayerData.job.name == "russian" then
        if OtherPlayer then
            ...
            TriggerEvent('qb-bossmenu:server:addAccountMoney', "russian", price)
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("info.fine_received", {fine = price}))
        end
    end
end)

RegisterNetEvent('russian:server:JailPlayer', function(playerId, time)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    ...
    if Player.PlayerData.job.name == "russian" then
        if OtherPlayer then
            ...
            TriggerClientEvent("russian:client:SendToJail", OtherPlayer.PlayerData.source, time)
            TriggerClientEvent('QBCore:Notify', src, Lang:t("info.sent_jail_for", {time = time}))
        end
    end
end)

RegisterNetEvent('russian:server:SetHandcuffStatus', function(isHandcuffed)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
...
-- RegisterNetEvent('russian:server:FlaggedPlateTriggered', function(camId, plate, street1, street2, blipSettings)
--     local src = source
...
--             if (Player.PlayerData.job.name == "russian" and Player.PlayerData.job.onduty) then
--                 if street2 then
--                     TriggerClientEvent("112:client:SendrussianAlert", v, "flagged", {
--                         camId = camId,
--                         plate = plate,
...
RegisterNetEvent('russian:server:SearchPlayer', function(playerId)
    local src = source
...
RegisterNetEvent('russian:server:SeizeCash', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if SearchedPlayer then
        ...
        SearchedPlayer.Functions.RemoveMoney("cash", moneyAmount, "russian-cash-seized")
...
RegisterNetEvent('russian:server:SeizeDriverLicense', function(playerId)
    local src = source
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if SearchedPlayer then
...
RegisterNetEvent('russian:server:RobPlayer', function(playerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if SearchedPlayer then
        local money = SearchedPlayer.PlayerData.money["cash"]
        Player.Functions.AddMoney("cash", money, "russian-player-robbed")
        SearchedPlayer.Functions.RemoveMoney("cash", money, "russian-player-robbed")
...
RegisterNetEvent('russian:server:UpdateBlips', function()
    -- KEEP FOR REF BUT NOT NEEDED ANYMORE.
end)

RegisterNetEvent('russian:server:spawnObject', function(type)
    local src = source
    local objectId = CreateObjectId()
    Objects[objectId] = type
    TriggerClientEvent("russian:client:spawnObject", src, objectId, type, src)
end)

RegisterNetEvent('russian:server:deleteObject', function(objectId)
    TriggerClientEvent('russian:client:removeObject', -1, objectId)
end)

RegisterNetEvent('russian:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    local src = source
...
RegisterNetEvent('russian:server:UpdateCurrentCops', function()
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "russian" and v.PlayerData.job.onduty then
    ...
    TriggerClientEvent("russian:SetCopCount", -1, amount)
end)
...
RegisterNetEvent('russian:server:showFingerprint', function(playerId)
    local src = source
    TriggerClientEvent('russian:client:showFingerprint', playerId, src)
    TriggerClientEvent('russian:client:showFingerprint', src, playerId)
end)

RegisterNetEvent('russian:server:showFingerprintId', function(sessionId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local fid = Player.PlayerData.metadata["fingerprint"]
    TriggerClientEvent('russian:client:showFingerprintId', sessionId, fid)
    TriggerClientEvent('russian:client:showFingerprintId', src, fid)
end)

RegisterNetEvent('russian:server:SetTracker', function(targetId)
    local src = source
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('russian:client:SetTracker', targetId, false)
    else
        ...
        TriggerClientEvent('russian:client:SetTracker', targetId, true)
    end
end)

RegisterNetEvent('russian:server:SyncSpikes', function(table)
    TriggerClientEvent('russian:client:SyncSpikes', -1, table)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent("russian:SetCopCount", -1, curCops)
    end
end)
